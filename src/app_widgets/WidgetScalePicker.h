...
public:
    ...
    ~WidgetScalePickerSlice() {
    }
    
};

class WidgetScalePicker : public WidgetMusical {
    
public:
    
    WidgetScalePicker(string persistentId, string widgetXML) : WidgetMusical(persistentId, widgetXML) {
        initWidget();
        loadState(widgetXML);
    }
    ...
    virtual ~WidgetScalePicker() {

      try {  
            if (scaleRulesMenuItem != nullptr) {
                ofRemoveListener(scaleRulesMenuItem->rulesChanged, this,  &WidgetScalePicker::onRulesChanged);
            }
            ...
            cout << "~WidgetScalePicker() error removing listeners. Have they already been removed? ";
        }

    }

    
    void setSliceProperties(WidgetScalePickerSlice *slice,
    ...
    void update(Aquamarine::WidgetContext context) override {
        ...
        for (int i = 1; i <= circleSlicesT1.size() ; i++) {
            ...
            WidgetScalePickerSlice *sliceT1 = &circleSlicesT1[i-1];
        ...
        for (int j = 1; j <= circleSlicesT2.size(); j++) {
            ...
            WidgetScalePickerSlice *sliceT2 = &circleSlicesT2[j-1];
    ...
    void drawSlice(int tier, int currentSlice, WidgetScalePickerSlice &slice) {
    ...
    virtual void setSelectedKey(vizNote val) override {
        ...
        if (tmpNotes.size() > 0) {
            ...
            WidgetMusical::setSelectedScale(targetScale, true);
    ...
    void setCalculatedScaleName(string val) override {
        WidgetMusical::setCalculatedScaleName(val);
...
private:
    Aquamarine::WidgetMenu *scalePickerContextMenu = nullptr;
    WidgetChordRules* scaleRulesMenuItem = nullptr;    
    ...
    vector<WidgetScalePickerSlice> circleSlicesT1;
    vector<WidgetScalePickerSlice> circleSlicesT2;
    ...
    WidgetScaleTable* scaleTableMenuItem;
    ...
    void initScale() {
        ...
        for (int degree=0; degree<notesT1.size(); degree++) {
            WidgetScalePickerSlice sliceT1;
        ...
        for (int key=0; key<notesT2.size(); key++) {
            WidgetScalePickerSlice sliceT2;
    ...
    vector<Aquamarine::WidgetMenuItem> getScalesMenuItems() {
        vector<Aquamarine::WidgetMenuItem> scalesMenuItems = { };
        vector<string> scales = MusicTheory::Scale::getAllKnownScales();
        
        for (int i=0; i<scales.size(); i++) {
            scalesMenuItems.push_back(
               Aquamarine::WidgetMenuItem(scales[i], i, false, i==0)
            );
        }
...
*/
        ...
        scaleTableMenuItem = new WidgetScaleTable(ofToUpper(getWidgetId()) + "_MENU_SCALE_TABLE", R"(
            <widget>
        ...
        scaleTableMenuItem->setSelectionMode(Aquamarine::WidgetTable::SelectionMode::ROW);
        scaleTableMenuItem->setAllowMultiSelect(false);
        ofAddListener(scaleTableMenuItem->cellSelected, this, &WidgetScalePicker::onScaleTableMenuItemSelected);
        ...
        if (!scalePickerContextMenu) {
            ...
            scaleRulesMenuItem = new WidgetChordRules(
            ...
            ofAddListener(scaleRulesMenuItem->rulesChanged, this,  &WidgetScalePicker::onRulesChanged);

            scalePickerContextMenu = new Aquamarine::WidgetMenu(getWidgetId() + "_SP_MENU1", "<widget><bounds width='100' height='300'/></widget>", getWidgetId()
            , Aquamarine::WidgetMenu::PreferredPopoutDirection::DOWN, {
                ...
                // Scales menu
                // ----------------------------------------------------------------------------
                Aquamarine::WidgetMenuTab(Aquamarine::Shared::lang("SCALES"), Aquamarine::IconCache::getIcon("MED_CONTENT_THEORY_SCALE"), 
                ...
                // Scale Rules menu
                // ----------------------------------------------------------------------------
                Aquamarine::WidgetMenuTab(Aquamarine::Shared::lang("SCALE_RULES"), Aquamarine::IconCache::getIcon("MED_CONTENT_RULES"),
            ...
            ofAddListener(dynamic_cast<Aquamarine::WidgetMenu*>(scalePickerContextMenu)->menuItemSelected, this, &WidgetScalePicker::onMenuItemSelected);

            Aquamarine::WidgetManager::addWidget(*scalePickerContextMenu, false, getWidgetId());
        }
    ...
    void onScaleTableMenuItemSelected(Aquamarine::WidgetTableArgs & args) {
    ...
    ofColor getLabelStateColor(WidgetScalePickerSlice slice) {
        if (slice.isHovered)  {
