...
class WidgetChordProgressionSliceNode {
public:
    int index = 0;
    int weight = 1;
    Aquamarine::Elm path;

    WidgetChordProgressionSliceNode(int index, int weight) : index(index), weight(weight) {};

    ~WidgetChordProgressionSliceNode() {
    }
    
};
class WidgetChordProgressionSlice {
public:
    vizChord chord;
    ...
    vector<WidgetChordProgressionSliceNode> nodes;
    ...
    ~WidgetChordProgressionSlice() {
    }
    
};

class WidgetChordProgression : public WidgetMusical {

public:
   
    WidgetChordProgression(string persistentId, string widgetXML) : WidgetMusical(persistentId, widgetXML) {
        initWidget();
        loadState(widgetXML);
    }
    ...
    virtual ~WidgetChordProgression() {
    ...
    void setSliceProperties(WidgetChordProgressionSlice *slice,
    ...
    void update(Aquamarine::WidgetContext context) override {
        ...
        for (int i = 1; i <= circleSlicesT1.size() ; i++) {
            ...
            WidgetChordProgressionSlice *sliceT1 = &circleSlicesT1[i-1];
       ...
       ofColor c = getSelectedFontColor(getLargeLabelText());
        ...
        for (int i = 1; i <= circleSlicesT1.size() ; i++) {
            WidgetChordProgressionSlice *slice = &circleSlicesT1[i-1];
                
            for (WidgetChordProgressionSliceNode& n:slice->nodes) {
                
                WidgetChordProgressionSlice *targetSlice = &circleSlicesT1[n.index-1];
    ...
    void drawSlice(int tier, int currentSlice, WidgetChordProgressionSlice &slice) {
    ...
    void draw(Aquamarine::WidgetContext context) override {
        ...
        for(int i=0; i<circleSlicesT1.size(); i++){
            if (circleSlicesT1[i].isHovered || (mAtLeastOneSliceHovered == false && i == 0)) {
                for (WidgetChordProgressionSliceNode& n:circleSlicesT1[i].nodes) {
                    n.path.draw();
                }
...
private:
    ...
    Widget *ChordProgressionContextMenu = nullptr;
    ...
    vector<WidgetChordProgressionSlice> circleSlicesT1;
    ...
    void initScale() {
        ...
        WidgetChordProgressionSlice slice_I, slice_II, slice_III, slice_IV, slice_V, slice_VI, slice_VII;
        ...
        for (int chordNum = 0; chordNum < progData.size(); chordNum++) {
            for (int chordDest: progData[chordNum]) {
                circleSlicesT1[chordNum].nodes.push_back(WidgetChordProgressionSliceNode(chordDest,1));
            }
        } 
    ...
    void initWidget() override {
        ...
        if (!ChordProgressionContextMenu) {
            ...
            ChordProgressionContextMenu = new Aquamarine::WidgetMenu(getWidgetId() + "_CP_MENU1", "<widget><bounds width='100' height='300'/></widget>", getWidgetId()
               ...
               // ----------------------------------------------------------------------------
                // Key Picker menu
                // ----------------------------------------------------------------------------
                Aquamarine::WidgetMenuTab(Aquamarine::Shared::lang("KEY_PICKER"), Aquamarine::IconCache::getIcon("MED_CONTENT_THEORY_LABEL"),
            ...
            ofAddListener(dynamic_cast<Aquamarine::WidgetMenu*>(ChordProgressionContextMenu)->menuItemSelected, this, &WidgetChordProgression::onMenuItemSelected);
            
            Aquamarine::WidgetManager::addWidget(*ChordProgressionContextMenu, false, getWidgetId());
        }
    ...
    ofColor getLabelStateColor(WidgetChordProgressionSlice slice) {
        if (slice.isHovered)  {
