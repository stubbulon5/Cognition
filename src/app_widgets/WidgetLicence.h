...
namespace Aquamarine
{
    struct Licence
    {
        ...
        static bool validateLicence(Licence &licence, bool fullValidation)
        {
            ...
            if (fullValidation)
            {
                ...
                licence.allowedAppMajorVersion = WidgetManager::getAppMajorVersion(LICENCE_valid_until_version);
                licence.allowedAppMinorVersion = WidgetManager::getAppMinorVersion(LICENCE_valid_until_version);

                int currentAppMajorVersion = WidgetManager::getAppMajorVersion(App::APPLICATION_VERSION());
                int currentAppMinorVersion = WidgetManager::getAppMinorVersion(App::APPLICATION_VERSION());
        ...
        static void saveLicenceToFile(string username, string password, string issued_licence, string token, uint64_t reauthorize_epoch)
        {
            ...
            string licence_raw_xml =
                "<licence>"
                "<username>" +
                Shared::encodeForXML(username) + "</username>"
                                                             "<password>" +
                Shared::encodeForXML(password) + "</password>"
                                                             "<issued_licence>" +
                Shared::encodeForXML(issued_licence) + "</issued_licence>"
                                                                   "<machineid>" +
                Shared::encodeForXML(Shared::getMachineId()) + "</machineid>"
                                                                                       "<token>" +
                Shared::encodeForXML(token) + "</token>"
                                                          "<reauthorize>" +
                ofToString(reauthorize_epoch) + "</reauthorize>"
                                                "</licence>";

            string encryptedLicenceXML = Shared::XOR_Encryption(licence_raw_xml, ENC_KEY(), true);
            // cout << "\n\nencrypted:" << encryptedLicenceXML;
            // string decryptedLicenceXML = Shared::XOR_Encryption(encryptedLicenceXML, ENC_KEY(), true);
        ...
        static Licence getLicence()
        {
            ...
            string licenceXML = Shared::XOR_Encryption(licenceEnc, ENC_KEY(), true);
        ...
        {
            if (!ofFile::doesFileExist(ENC_KEY_FILE(), true))
                return "";
    ...
    class WidgetLicence : public Widget
    {

    public:
        WidgetLicence(string persistentId, string widgetXML) : Widget(persistentId, widgetXML)
        {
            initWidget();
        }

        virtual ~WidgetLicence()
        {
        }
        ...
        void draw(WidgetContext context) override
        {
            if (keyIcon.getIsVisible())
            {
                ...
                Coord::vizBounds iconBounds = keyIcon.getScaledBounds();
            ...
            if (event == "OK." + WIDGET_EVENT::CLICK)
            {
                ...
                if (TXT_LICENCE)
                {
                    ...
                    else
                    {
                        ...
                        Licence licence = Licence::getLicence();
                        if (licence.isValid)
                            Licence::validateLicence(licence, true); // Full validation
        ...
        {
            WidgetElmLabel *LBL_VALIDATION_RESULT = getLabel("LBL_VALIDATION_RESULT");
            if (licence.isValid)
            {
    ...
    private:
        ...
        void initWidget() override
        {
            ...
            keyIcon = IconCache::getIcon("MED_MENU");
            keyIcon.setColor(ofColor(255, 255, 255, 200));
            keyIcon.setIsVisible(false);

            if (!loaded)
            {
                WidgetManager::loadWidgetFromFileToExisting("ui/widgets/licence.xml", *this);
                loaded = true;
            }
            ...
            if (TXT_USERNAME)
                TXT_USERNAME->setValue(licence.username, true);
            if (TXT_PASSWORD)
                TXT_PASSWORD->setValue(licence.password, true);
            if (TXT_USERNAME)
                TXT_LICENCE->setValue(licence.issued_licence, true);
