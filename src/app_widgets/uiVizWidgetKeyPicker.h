...
public:
    ...
    ~uiVizWidgetKeyPickerSlice() {
    }
};

class uiVizWidgetKeyPicker :  public uiVizWidgetMusical {

public:
   
    
    
    uiVizWidgetKeyPicker(string persistentId, string widgetXML) : uiVizWidgetMusical(persistentId, widgetXML) {
        initWidget();
        loadState(widgetXML);
    }
    ...
    void update(uiVizWidgetContext context) override {
        ...
        for (int i = 1; i <= circleSlices.size() ; i++) {
            ...
            uiVizWidgetKeyPickerSlice* slice = &circleSlices[i-1];
...
private:
   ...
   //     vector<string> keys = getChromaticScaleSharps();
    vector<uiVizWidgetKeyPickerSlice> circleSlices;
    ...
    void initWidget() override {
        ...
        for (int i = 1; i <= keys.size() ; i++) {
            uiVizWidgetKeyPickerSlice slice;// = new uiVizWidgetKeyPickerSlice();
    ...
    ofColor getLabelStateColor(uiVizWidgetKeyPickerSlice slice) {
