...
class WidgetChordDesignerSlice {
public:
    vizNote note;
    Aquamarine::Elm path;
    string label;
    ...
    ~WidgetChordDesignerSlice() {
    }
    
};

class WidgetChordDesigner : public WidgetMusical {
    
public:
    WidgetChordDesigner(string persistentId, string widgetXML) : WidgetMusical(persistentId, widgetXML) {
        initWidget();
        loadState(widgetXML);
    }
    ...
    virtual ~WidgetChordDesigner() {
      ...
      try {  
            if (chordRulesMenuItem != nullptr) {
                ofRemoveListener(chordRulesMenuItem->rulesChanged, this,  &WidgetChordDesigner::onRulesChanged);
            }
            
            if (chordDesignerContextMenu) {
                ofRemoveListener(dynamic_cast<Aquamarine::WidgetMenu*>(chordDesignerContextMenu)->menuItemSelected, this, &WidgetChordDesigner::onMenuItemSelected);
                chordDesignerContextMenu->deleteThisWidget();
            }
        } catch(...) {
            cout << "~WidgetChordDesigner() error removing listeners. Have they already been removed? ";
        }
    ...
    void setSliceProperties(WidgetChordDesignerSlice *slice,
    ...
    void update(Aquamarine::WidgetContext context) override {
        ...
        for (int i = 1; i <= circleSlicesT0.size() ; i++) {
            ...
            WidgetChordDesignerSlice *sliceT0 = &circleSlicesT0[i-1];
        ...
        for (int i = 1; i <= circleSlicesT1.size() ; i++) {
            ...
            WidgetChordDesignerSlice *sliceT1 = &circleSlicesT1[i-1];
        ...
        for (int j = 1; j <= circleSlicesT2.size(); j++) {
            ...
            WidgetChordDesignerSlice *sliceT2 = &circleSlicesT2[j-1];
    ...
    void drawSlice(int tier, int currentSlice, WidgetChordDesignerSlice &slice) {
...
/*
    ...
    virtual void setIsExtendedChordMode(bool val) override {
        WidgetMusical::setIsExtendedChordMode(val);
    ...
    void addSelectedChord(vizChord &rawChord, bool onlyAddIfNotFound, bool includeInstrumentRules) override {
        ...
        clearSelectedNotes();
        WidgetMusical::setSelectedKey(rawChord.getChordRootNote());
        initScale();
    ...
    void setCalculatedChordName(string val) override {
        WidgetMusical::setCalculatedChordName(val);
    ...
    WidgetChordRules* getChordRulesMenuItem() {
...
private:
    Aquamarine::Widget *chordDesignerContextMenu = nullptr;
    ...
    vector<WidgetChordDesignerSlice> circleSlicesT0;
    vector<WidgetChordDesignerSlice> circleSlicesT1;
    vector<WidgetChordDesignerSlice> circleSlicesT2;
    ...
    void initScale() {
        ...
        for (int degree=0; degree<notesT0.size(); degree++) {
            WidgetChordDesignerSlice sliceT0;
        ...
        for (int degree=0; degree<notesT1.size(); degree++) {
            WidgetChordDesignerSlice sliceT1;
        ...
        for (int key=0; key<notesT2.size(); key++) {
            WidgetChordDesignerSlice sliceT2;
    ...
    virtual void setTitle(string title) override {
        if(getUserDefinedChordName() != "") {
            Widget::setTitle(getSelectedKey().getNoteName() + getUserDefinedChordName());
        } else {
            Widget::setTitle(title);
        }
    }

    vector<Aquamarine::WidgetMenuItem> getChordsMenuItems() {
        vector<Aquamarine::WidgetMenuItem> chordsMenuItems = { };
        vector<string> chords = MusicTheory::Chord::getAllKnownChords();
        
        for (int i=0; i<chords.size(); i++) {
            chordsMenuItems.push_back(
               Aquamarine::WidgetMenuItem(chords[i], i, false, i==0)
            );
        }
    ...
    void initWidget() override {
        ...
        if (!userDefinedChordName) {
            userDefinedChordName = dynamic_cast<Aquamarine::WidgetElmTextbox*>(addOrLoadWidgetElement(userDefinedChordName, Aquamarine::WIDGET_ELM_CLASS::TEXTBOX, getPersistentId() + "_customChordName", R"(
                <element>
                </element>
                )"));
        ...
        if (getSelectedKey().getNoteName() == "") {
            ...
            WidgetMusical::setSelectedKey(defaultKey);
        ...
        if (!chordDesignerContextMenu) {
            ...
            ofAddListener(chordRulesMenuItem->rulesChanged, this,  &WidgetChordDesigner::onRulesChanged);

            chordDesignerContextMenu = new Aquamarine::WidgetMenu(getWidgetId() + "_CD_MENU1", "<widget><bounds width='100' height='300'/></widget>", getWidgetId()
            , Aquamarine::WidgetMenu::PreferredPopoutDirection::DOWN, {
                ...
                // Chords menu
                // ----------------------------------------------------------------------------
                Aquamarine::WidgetMenuTab(Aquamarine::Shared::lang("CHORDS"), Aquamarine::IconCache::getIcon("MED_CONTENT_THEORY_SCALE"), MENU_GROUP_1::MENU_TAB_CHORD,
                                    getChordsMenuItems()
                                    ),
            ...
            ofAddListener(dynamic_cast<Aquamarine::WidgetMenu*>(chordDesignerContextMenu)->menuItemSelected, this, &WidgetChordDesigner::onMenuItemSelected);
    ...
    int getUsableHeight() override {
        if(userDefinedChordName && userDefinedChordName->getIsVisible()) {
            return WidgetMusical::getUsableHeight() - userDefinedChordName->getHeight() - getScaledPadding();
        } else {
            return WidgetMusical::getUsableHeight();
        }
    }

    int getUsableY_Custom() {
        if(userDefinedChordName && userDefinedChordName->getIsVisible()) {
            return WidgetMusical::getUsableY() + userDefinedChordName->getHeight() + getScaledPadding();
        } else {
            return WidgetMusical::getUsableY();
        }
    }
    ...
    ofColor getLabelStateColor(WidgetChordDesignerSlice slice) {
        if (slice.isHovered)  {
