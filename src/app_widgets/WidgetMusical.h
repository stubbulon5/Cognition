...
class WidgetMusical :  public Aquamarine::Widget {
...
public:
    ...
    WidgetMusical(string persistentId, string widgetXML) : Widget(persistentId, widgetXML) {
        init();
        loadState(widgetXML);
    }
    ...
    Aquamarine::WidgetElmBreadcrumb* getWidgetBreadCrumbBar() {
        if (mWidgetElmBreadcrumb == nullptr && getBreadcrumbItemItems().size() > 0) {
            mWidgetElmBreadcrumb = dynamic_cast<Aquamarine::WidgetElmBreadcrumb*>(addOrLoadWidgetElement(mWidgetElmBreadcrumb, Aquamarine::WIDGET_ELM_CLASS::BREADCRUMB, "_widgetBreadcrumb", R"(
                <element id="___TOP_BREADCRUMB" class="WidgetElmBreadcrumb" shouldPersist="0">
    ...
    void setBreadcrumbItemItems(vector<Aquamarine::Aquamarine::WidgetElmBreadcrumbItem> items) {
    ...
    void setBreadcrumbItemItemsVisibility(bool visible) {
        Aquamarine::WidgetElmBreadcrumb* bar = getWidgetBreadCrumbBar();
        if(bar) {
            bar->setIsVisible(visible);
        }
    }    

    vector<Aquamarine::Aquamarine::WidgetElmBreadcrumbItem> getBreadcrumbItemItems() {
        return mBreadcrumbItemItems;
    }   

    Aquamarine::WidgetEventListener* getWidgetEventListenerConfig() {
        if (mWidgetEventListenerConfig == nullptr) {
            mWidgetEventListenerConfig = new Aquamarine::WidgetEventListener(
    ...
    int getUsableHeight() override {
        ...
        if(bar && bar->getIsVisible() && !getIsMinimized()) {
            return Aquamarine::Widget::getUsableHeight() - bar->getHeight() - getScaledPadding();
        } else {
            return Aquamarine::Widget::getUsableHeight();
        }
    }
    ...
    int getUsableY() override {
        Aquamarine::WidgetElmBreadcrumb* bar = getWidgetBreadCrumbBar();
        if(bar && bar->getIsVisible() && !getIsMinimized()) {
            return Aquamarine::Widget::getUsableY() + bar->getHeight() + getScaledPadding();
        } else {
            return Aquamarine::Widget::getUsableY();
        }
    }    

     
    virtual bool loadState(string widgetXML) override {
        Aquamarine::Widget::loadState(widgetXML);
    ...
    void onWidgetMouseContentDragReleased(ofMouseEventArgs &e, string dragData) override {
        ...
        Widget* widgetTarget = Aquamarine::WidgetManager::getWidget(getTargetDropWidgetId());
        WidgetMusical* musicalWidgetTarget = dynamic_cast<WidgetMusical*>(Aquamarine::WidgetManager::getWidget(getTargetDropWidgetId()));
    ...
    static void populateTargetMusicalWidget(WidgetMusical* musicalWidgetTarget, string dragData) {
    ...
    Aquamarine::WidgetMenu* getPopoutMenu(int menuTag) {
        Widget* widget = getPopoutWidgetForMenuTag(menuTag);
        if (widget == nullptr) return nullptr;
        Aquamarine::WidgetMenu* widgetMenu = dynamic_cast<Aquamarine::WidgetMenu*>(widget);
        return widgetMenu;
    }
    ...
    virtual vector<Aquamarine::Icon> getWidgetTitleIcons() override {
        vector<Aquamarine::Icon> icons = Widget::getWidgetTitleIcons();
    ...
    virtual void titleIconClicked(int iconTag, Aquamarine::Coord::vizBounds deScaledClickBounds) override {
        ...
        if (iconTag == Aquamarine::IconCache::IconTag::WIDGET_HELP) {
            if (mWidgetTextEditorHelp == nullptr) {
                mWidgetTextEditorHelp = new Aquamarine::WidgetDialog(getPersistentId() + "__HELP", R"(
                <widget>
...
private:
    ...
    Aquamarine::WidgetElmBreadcrumb *mWidgetElmBreadcrumb = nullptr;
    vector<Aquamarine::Aquamarine::WidgetElmBreadcrumbItem> mBreadcrumbItemItems;

    Aquamarine::WidgetDialog *mWidgetTextEditorHelp = nullptr;
    Aquamarine::WidgetEventListener *mWidgetEventListenerConfig = nullptr;
    ...
    void init() {
        ...
        m_selectedScale = vizScale();
        m_draggingScale = vizScale();
        // setBreadcrumbItemItems({WidgetElmBreadcrumbItem("a", "A123"), WidgetElmBreadcrumbItem("b", "Bsfsdf"), WidgetElmBreadcrumbItem("c", "Csdgasasfd")});
    }
