...
public:
    ...
    void showClipboardMenu(int x, int y, string clipboardOperationTargetWidgetId) {
        ...
        if (clipboardMenu->getIsVisible()) return;
    ...
    void hideClipboardMenu(bool force) {
        ...
        if (!force && !mMenuHasBeenHovered) return;
...
private:
    ...
    void initWidget() override {
        ...
        if (!clipboardMenu) {
            clipboardMenu = new uiVizWidgetMenu("CLIPBOARD_MENU", "<widget isSystemWidget='1'><bounds width='100' height='100'/></widget>", getWidgetId()
                , uiVizWidgetMenu::PreferredPopoutDirection::DOWN, {
                    // ----------------------------------------------------------------------------
                    // Clipboard...
                    // ----------------------------------------------------------------------------
                    uiVizWidgetMenuTab("Clipboard", uiVizIconCache::getIcon("MED_WIDGET_CLIPBOARD"),
                                    uiVizIconCache::IconTag::WIDGET_CLIPBOARD, {
                                        uiVizWidgetMenuItem("Cut", 0),
                                        uiVizWidgetMenuItem("Copy", 1),
                                        uiVizWidgetMenuItem("Paste", 2)
                                    }),
                });
