...
public:
    ...
    uiVizWidgetMenuTab(string label, uiVizIcon icon, int uniqueID, uiVizWidget* tabWidget) : label(label), icon(icon), uniqueID(uniqueID), tabWidget(tabWidget) {
        if (tabWidget) tabWidgetOriginalBounds = uiVizCoord::vizBounds(tabWidget->getX(), tabWidget->getY(), tabWidget->getWidth(), tabWidget->getHeight());
    };
...
public:
    ...
    uiVizWidgetMenu(string persistentId, string widgetXML, string ownerWidgetId, PreferredPopoutDirection preferredDirection, vector<uiVizWidgetMenuTab> menuTabs) : uiVizWidget(persistentId, widgetXML, ownerWidgetId) {
        mMenuTabs = menuTabs;
        if (mMenuTabs.size() > 0) setActiveMenuTabIndex(0);
        initWidget();
    }
    ...
    void update(uiVizWidgetContext context) override {
        if (menuSliceData.size() == 0) initializeMenuSlices(false);
        ...
        if (menuSliceData.size() > 0)  {
            for (int i = 0; i < menuSliceData[mActiveMenuTabIndex].size() ; i++) {
                ...
                if (i == 0) appendContentBoundsScaled(slice->bounds); // Add one extra item height
...
//int extraWidth = getWidth() - getUsableWidth();
        ...
        if (getIsTabbedMenu()) {
            ...
            for (int i = 0; i < tabSlices.size(); i++) {
                ...
                if (slice->menuTab.isSelected || sliceHovered || slice->currentAlpha > 0) {
                    ...
                    slice->currentAlpha = !slice->isHovered() ? scaleAnimation(getWidgetId() + "_tabhovered_" + ofToString(i), slice->currentAlpha, 0, 0.3f) :
                    scaleAnimation(getWidgetId() + "_tabhovered_" + ofToString(i), slice->currentAlpha, targetTabHoveredAlpha, 0.05f);
            ...
            for (int i = 0; i < menuSliceData[mActiveMenuTabIndex].size(); i++) {
                ...
                if (isInsideWidgetBounds(deScale(slice->bounds.x), deScale(slice->bounds.y), deScale(slice->bounds.width), deScale(slice->bounds.height))) {
                    ...
                    if (sliceEnabled && (menuItem->isSelected || sliceHovered || slice->currentAlpha > getTheme().GenericSelectedAlpha)) {
                        ...
                        slice->currentAlpha = !menuItem->isSelected && !slice->isHovered() && slice->currentAlpha > getTheme().GenericSelectedAlpha ?
                            scaleAnimation(getWidgetId() + "_hovered_" + ofToString(i), slice->currentAlpha, getTheme().GenericSelectedAlpha, 0.3f) :
                            scaleAnimation(getWidgetId() + "_hovered_" + ofToString(i), slice->currentAlpha, getTheme().GenericHoveredAlpha, 0.05f);
            
                        if (sliceHovered && getIsMousePressedOverWidget(true)) slice->currentAlpha = getTheme().GenericHoveredAlpha;
    ...
    void resizeToTabWidget(int width, int height) {
        ...
        if (tabSlice != nullptr && tabSlice->menuTab.tabWidget)  {
            ...
            if (targetWidth < tabWidgetMinWidth) targetWidth = tabWidgetMinWidth;
            if (targetHeight < tabWidgetMinHeight) targetHeight = tabWidgetMinHeight;
...
// add to chord wheel - clicked - add for fretboard
    ...
    virtual vector<uiVizIcon> getWidgetTitleIcons() override {
        ...
        if (getShowMenuPin()) {
            return {
                ...
                getIsMenuPinned() ? uiVizIconCache::getIcon("REG_REG_RADIO_BUTTON_CHECKED") :
    ...
    void setMenuItemSelectedVal(int tabMenuId, int menuItemId, bool selected, bool fireEvent) {
        ...
        if (tabIndex == -1 || menuItemIndex == -1) return;
        ...
        if (menuItem->isToggle) {
            ...
            for (int j = 0; j< menuSliceData[tabIndex].size(); j++) {
                ...
                if (!unselectSlice->menuItem.isToggle) unselectSlice->menuItem.isSelected = false;
            }
            menuItem->isSelected = selected;
        }
    ...
    int getUsableX(bool inclScrollOffset) override {
        return (getTabLocation() == TabLocation::LEFT) ?
            uiVizWidget::getUsableX(inclScrollOffset) + getTabWidth() + getNonScaledPadding() : uiVizWidget::getUsableX(inclScrollOffset);
    }
    
    int getUsableY(bool inclScrollOffset) override {
        return (getTabLocation() == TabLocation::TOP) ?
            uiVizWidget::getUsableY(inclScrollOffset) + getTabHeight()  + getNonScaledPadding() : uiVizWidget::getUsableY(inclScrollOffset);
    }
    
    int getUsableWidth() override {
        return (getTabLocation() == TabLocation::LEFT || getTabLocation() == TabLocation::RIGHT) ?
            uiVizWidget::getUsableWidth() - getTabWidth() - getNonScaledPadding() : uiVizWidget::getUsableWidth();
    }
    
    int getUsableHeight() override {
        return (getTabLocation() == TabLocation::TOP || getTabLocation() == TabLocation::BOTTOM) ?
            uiVizWidget::getUsableHeight() - getTabHeight() - getNonScaledPadding() : uiVizWidget::getUsableHeight();
    }
    ...
    int getIndexForTabMenu(int tabMenuId) {
        for (int i=0; i< tabSlices.size(); i++) {
            uiVizWidgetTabSlice* slice = &tabSlices[i];
            if ( slice != nullptr && tabMenuId == slice->menuTab.uniqueID)  return i;
        }
        return -1;
    }
    ...
    int getIdForTabMenuIndex(int tabMenuIndex) {
        for (int i=0; i< tabSlices.size(); i++) {
            uiVizWidgetTabSlice* slice = &tabSlices[i];
            if ( slice != nullptr && tabMenuIndex == i)  return slice->menuTab.uniqueID;
        }
        return -1;
    }
    ...
    int getIndexForTabMenuItem(int tabIndex, int menuItemId) {
        ...
        if (idForTabMenu == -1) return -1;
        if (mMenuTabs[tabIndex].menuItems.size() == 0) return -1;
        
        for (int i=0; i< mMenuTabs[tabIndex].menuItems.size(); i++) {
            if (menuItemId == mMenuTabs[tabIndex].menuItems[i].uniqueID)  return i;
        }
        return -1;
    }
