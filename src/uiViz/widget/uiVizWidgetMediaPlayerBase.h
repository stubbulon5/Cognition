...
public:
    ...
    virtual void update(uiVizWidgetContext context) override {
        ...
        if (getIsPlaying()) updateMediaPlayer();
        ...
        if (isWidgetHovered()) {
            ...
            if (getShowPlaybackButtons()) {
                ...
                if (getIsPlaying()) {
                    if (play_pause->getIconTag() != "REG_REG_PAUSE_BUTTON") play_pause->setIconTag("REG_REG_PAUSE_BUTTON");
                } else {
                    if (play_pause->getIconTag() != "REG_REG_PLAY_BUTTON") play_pause->setIconTag("REG_REG_PLAY_BUTTON");
                }
            }
        } else {
            // Scrubber
        ...
        if (isPlaying) setNeedsUpdate(true);
    }
   ...
   virtual void onWidgetEventReceived(uiVizWidgetEventArgs &args) override {
        ...
        if (args.eventName == WIDGET_EVENT::VALUE_CHANGED && scrubber && args.sender.getPersistentId() == scrubber->getPersistentId()) {
            ...
            if (!getDoesRespondToFileDrop()) return;            
