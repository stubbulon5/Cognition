...
public:
    ...
    uiVizWidgetTableCell(string key, string label, int width) : key(key), label(label), width(width) { if (width > 10) mWasWidthSet = true; };
    uiVizWidgetTableCell(string key, string label, int width, string horizontalAlign) : key(key), label(label), horizontalAlign(horizontalAlign), width(width) { if (width > 10) mWasWidthSet = true; };
    uiVizWidgetTableCell(string key, string label, string horizontalAlign, int width, string WIDGET_CLASS) : key(key), label(label), horizontalAlign(horizontalAlign), width(width), WIDGET_CLASS(WIDGET_CLASS) { if (width > 10) mWasWidthSet = true;};

  
    uiVizWidgetTableCell(string key, string label, int widthPerc, int minWidth) : key(key), label(label), widthPerc(widthPerc), width(minWidth) { if (widthPerc > 0) mWasWidthPercSet = true; }; //! Use minWidth = 0 to set min width to Cell label width
    uiVizWidgetTableCell(string key, string label, string horizontalAlign, int widthPerc, int minWidth) : key(key), label(label), horizontalAlign(horizontalAlign), widthPerc(widthPerc), width(minWidth) { if (widthPerc > 0) mWasWidthPercSet = true; }; //! Use minWidth = 0 to set min width to Cell label width
    uiVizWidgetTableCell(string key, string label, string horizontalAlign, int widthPerc, int minWidth, string WIDGET_CLASS) : key(key), label(label), horizontalAlign(horizontalAlign), widthPerc(widthPerc), width(minWidth), WIDGET_CLASS(WIDGET_CLASS) { if (widthPerc > 0) mWasWidthPercSet = true;}; //! Use minWidth = 0 to set min width to Cell label width
...
public:
    ...
    int calculateNumberOfPercCells() {
        int val = 0;
        for (uiVizWidgetTableCell& cell:cells) {            
            if (cell.getWasWidthPercSet()) val++;
        }   
        return val;     
    }  
...
public:
    ...
    string getSelectionModeString(SelectionMode val) {
        switch(val) {
            case SelectionMode::ROW: return "ROW";
            case SelectionMode::CELL: return "CELL";
    ...
    SelectionMode getSelectionModeEnum(string val) {
        val = ofToUpper(val);
        if (val == "ROW") return SelectionMode::ROW;
        if (val == "CELL") return SelectionMode::CELL;
        if (val == "NONE") return SelectionMode::NONE;
    ...
    virtual bool loadState(string widgetXML) override {
        ...
        setCellLabelColor(cellLabelColor_RGBA != "" ? 
           uiVizWidgetTheme::getColor_fromRGBAHex(cellLabelColor_RGBA) : getCellLabelColor()
        );

        string cellLabelSelectedColor_RGBA = settings.getAttribute("properties", "cellLabelSelectedColor", "");
        setCellLabelSelectedColor(cellLabelSelectedColor_RGBA != "" ? 
           uiVizWidgetTheme::getColor_fromRGBAHex(cellLabelSelectedColor_RGBA) : getCellLabelSelectedColor()
        );

        string cellHoverColor_RGBA = settings.getAttribute("properties", "cellHoverColor", "");
        setCellHoverColor(cellHoverColor_RGBA != "" ? 
           uiVizWidgetTheme::getColor_fromRGBAHex(cellHoverColor_RGBA) : getCellHoverColor()
        );        

        string cellSelectionColor_RGBA = settings.getAttribute("properties", "cellSelectionColor", "");
        setCellSelectionColor(cellSelectionColor_RGBA != "" ? 
           uiVizWidgetTheme::getColor_fromRGBAHex(cellSelectionColor_RGBA) : getCellSelectionColor()
        );        
    ...
    void update(uiVizWidgetContext context) override {
      ...
      //  if (getShowColumnFilters()) hideColumnFilters();
        ...
        if (getShowColumnFilters() && !getIsPivoted()) {
            ...
            if (TXT_COL_FILTER) colFilterHeight = TXT_COL_FILTER->getHeight();
        ...
        if (getTableUseFbo()) clearOutOfViewCellFbo();
        ...
        if (getFreezeFirstColumn() && visibleColIndexStart > 0) calculateHeaderCellBounds(0, accumX, mMaxHeaderRowHeight, labelHeight);
        ...
        for (int i = visibleRowIndexStart; i < visibleRowIndexEnd; i++) {            
            ...
            for(int j=visibleColIndexStart; j<visibleColIndexEnd; j++) {
                ...
                if (j==visibleColIndexStart && getFreezeFirstColumn() && visibleColIndexStart > 0) dynamicallySizeCol(row, 0, accumX, cellWidth, rowHeight, labelHeight);
            }
        }
    ...
    void draw(uiVizWidgetContext context) override {
                ...
                if ((row.isSelected || clicked || isRowHovered) && !getIsDragging()) {
                    ...
                    if (isCellHovered) {                     
                        if (mHoveredColIndex > -1 && mHoveredRowIndex > -1 && (mHoveredColIndexPrev != mHoveredColIndex || mHoveredRowIndexPrev != mHoveredRowIndex)) {
                            ...
                            if (mFunction_Cell_Hovered != NULL) mFunction_Cell_Hovered(args);
                        }
                        mHoveredColIndexPrev = mHoveredColIndex; mHoveredRowIndexPrev = mHoveredRowIndex;
                    }
                    ...
                    if (!eventFiredForRow && clicked && isCellHovered) {
                        ...
                        if ((!row.isSelected && getSelectionMode() == SelectionMode::ROW) || (!cell.isSelected && getSelectionMode() == SelectionMode::CELL)) {
                            ...
                            if (mFunction_Cell_Selected != NULL) mFunction_Cell_Selected(args);

                            if (getShouldCloseUponSelection()) closeThisWidget();
                        } else {
                            ...
                            if (mFunction_Cell_UnSelected != NULL) mFunction_Cell_UnSelected(args);                            
                        }
        ...
        if (getFixedHeaderRowHeight() != 0 || getFixedFooterRowHeight() > 0) {
            ...
            for(int j = visibleColIndexEnd-1; j >= visibleColIndexStart; j--) { // iterate backwards (so 1st header cell drawn last)!!!
                if (getFixedHeaderRowHeight() != 0) drawHeaderRow(j, rowHovered, row);
                if (getFixedFooterRowHeight() > 0) drawFooterRow(j, rowHovered, row);
            }
            if (getFreezeFirstColumn() && visibleColIndexStart > 0) {
                if (getFixedHeaderRowHeight() != 0) drawHeaderRow(0, rowHovered, row);
                if (getFixedFooterRowHeight() > 0) drawFooterRow(0, rowHovered, row);
            }
        }
    ...
    void pivot() {
        ...
        for (int x=0; x<mHeaderRow.cells.size(); x++) {
            ...
            for (int i=0; i< mTableRows.size(); i++) {    
                if (x > mTableRows[i].cells.size()-1) break;
        ...
        for (int i=0; i<= mTableRows.size(); i++) { // not sure why the extra col is needed to make last visible in pivot mode?!?!
    ...
    vector<int> getSelectedRowIndexes() {
        ...
        for(int i=0; i<mTableRows.size(); i++) {
            ...
            if (row.isSelected) selectedRowIndexes.push_back(i);
        }
    ...
    vector<uiVizWidgetTableRow> getSelectedRows() {
        ...
        for(int i=0; i<selectedRowIndexes.size(); i++) {
            ...
            if (row.isSelected) selectedRows.push_back(row);
        }
        return selectedRows;
    }    
    ...
    void filterTable(vector<uiVizWidgetTableFilterData> columnFilters) {
        ...
        for(uiVizWidgetTableRow row:mTableRowsFull) {
            for(uiVizWidgetTableFilterData filter:columnFilters) {
                ...
                if (filter.columnIndex > row.cells.size()-1) continue;
    ...
    void filterTable(int columnIndex, string filterValue) {       
        ...
        if (!foundFilter) mColumnFilters.push_back(newFilter);
    ...
    void setShowColumnFilters(bool val, bool addElms) {
        ...
        if (mShowColumnFilters && getFixedHeaderRowHeight() != 0) {
            for (int i=0; i<mHeaderRow.cells.size(); i++) {
                ...
                if (e) e->setTitle(mHeaderRow.cells[i].label + "...");
            }
        } else {
            for (int i=0; i<mHeaderRow.cells.size(); i++) {
                uiVizWidgetElm* e = getWidgetElement("COL_FILTER_" + ofToString(i));
                if (e) removeWidgetElement(e);
            }            
        }
    ...
    uiVizWidgetTableCell getHeaderCellByIndex(int index) {
        return (index > -1 && index < mHeaderRow.cells.size() ? 
           mHeaderRow.cells[index] : uiVizWidgetTableCell("NOT_FOUND", "")
        );
    }
    ...
    void selectCellByIndex(int rowIndex, int cellIndex, bool scrollToRow, bool scrollToCell, bool fireEvent) {
        if(rowIndex > mTableRowsFull.size()-1) return;
    ...
    void selectRowByIndex(int rowIndex, bool scrollToRow, bool fireEvent) {
        if(rowIndex > mTableRows.size()-1) return;
...
protected:
    ...
    void populateTableTheme() {
        if (!mCellLabelColorSetManually) mCellLabelColor = getTheme().TypographySecondaryColor;
        if (!mCellLabelSelectedColorSetManually) mCellLabelSelectedColor = getTheme().SelectionForegroundColor;
        if (!mCellHoverColorSetManually) mCellHoverColor = getTheme().HoverBackgroundColor;
        if (!mCellSelectionColorSetManually) mCellSelectionColor = getTheme().SelectionBackgroundColor;
        if (!mRowBGColorSetManually) mRowBGColor = getTheme().WidgetAlternating1Color;
        if (!mRowAltBGColorSetManually) mRowAltBGColor = getTheme().WidgetAlternating2Color;
    }
...
private:
    ...
    int calculateVisibleColIndexEnd() {
        ...
        for(int j=visibleColStartOffset.index; j<mHeaderRow.cells.size(); j++) {
            if ((deScale(accumWidthEnd) >= (getWidth()) + deScale(visibleColStartOffset.offset) + (getFreezeFirstColumn() ?  deScale(mHeaderRow.cells[0].cellSlice.bounds.width) : 0))) break;
    ...
    int calculateHeaderCellBounds(int colIndex, int accumX, int titleRowHeight, int labelHeight) {
        ...
        if (cell.label != "") rectLabel = font->rect(cell.label);
    ...
    int dynamicallySizeCol(uiVizWidgetTableRow& row, int colIndex, int accumX, int cellWidth, int rowHeight, int labelHeight) {
        if (colIndex > row.cells.size()-1) return 0;
    ...
    int calculateWidthPerc(uiVizWidgetTableCell& cell, ofRectangle rectLabel) {
        ...
        int cellWidthNonPercVal = cell.getWidth() <= 0 ? 
            std::max((int)10, (int)(deScale(rectLabel.width) + getNonScaledPadding() * 2)) : cell.getWidth();
