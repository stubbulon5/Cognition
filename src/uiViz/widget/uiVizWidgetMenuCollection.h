...
public:
    ...
    void init() {
        if (displayOptions.nonScaledPadding.left == 0) displayOptions.nonScaledPadding.left = getDefaultNonScaledPadding();
        if (displayOptions.nonScaledPadding.right == 0) displayOptions.nonScaledPadding.right = getDefaultNonScaledPadding();
    }
...
public:
    ...
    MenuType getMenuTypeEnum(string val) {
        val = ofToUpper(val);
        if (val == "MENU") return MenuType::MENU;
        if (val == "TAB") return MenuType::TAB;
    ...
    void setTabMenuCollection(uiVizWidget* owner, vector<uiVizWidgetMenuCollectionItem> menuCollection) {
        if (owner) setOwnerWidgetId(owner->getWidgetId());
        mMenuCollection = menuCollection;  
        if(getMenuType() == MenuType::TAB && !isAnySliceSelected() && mMenuCollectionSliceData.size() > 0) {
            setSelectedSlice(mMenuCollectionSliceData[0], false);
        }        
    ...
    uiVizWidgetMenuCollectionItemSlice* getSelectedSlice() {
        for (uiVizWidgetMenuCollectionItemSlice& slice:mMenuCollectionSliceData) {
            if (slice.collectionItem.isSelected) return &slice;
        }
        return nullptr;
    }
    ...
    int getSelectedSliceIndex() {
        int i=0;
        for (uiVizWidgetMenuCollectionItemSlice slice:mMenuCollectionSliceData) {
            if (slice.collectionItem.isSelected) return i;
            i++;
        }
        return -1;
    }    
    ...
    bool isAnySliceSelected() {
        for (uiVizWidgetMenuCollectionItemSlice& slice:mMenuCollectionSliceData) {
            if (slice.collectionItem.isSelected) return true;
        }
        return false;
    }
    ...
    bool isAnyMenuVisible() {
        for (uiVizWidgetMenuCollectionItemSlice& slice:mMenuCollectionSliceData) {
            if (slice.collectionItem.menu && slice.collectionItem.menu->getIsVisible()) return true;
        }
        return false;
    }    
    ...
    clearSelectedSlice();
            ...
            if(getMenuType() == MenuType::TAB) {
                ...
                if (owner) w = owner->getChildWidgetByPersistentId(slice.collectionItem.widgetPersistentId);
                if (!w) w = slice.collectionItem.menu;
            } else {
    ...
    virtual ~uiVizWidgetMenuCollection() {
        ...
        for (int i=0; i<mMenuCollectionSliceData.size(); i++) {
            ...
            if (w) {
                ...
                if (getMenuType() == MenuType::TAB) {
                    if (owner) owner->removeChildWidgetByPersistentId(w->getPersistentId());
                } else {
                    w->setOwnerWidgetId("");
                    w->deleteThisWidget();
                }
	...
	virtual void onWidgetAfterAllWidgetsLoaded(std::vector<std::reference_wrapper<uiVizWidget>> widgets) {
        ...
        if (mMenuCollectionSliceData.size() == 0) initializeMenuSlices();
	}
	...
	virtual void onWidgetOwnedWidgetClosed(uiVizWidgetBase* closedWidget) override {
        if (getMenuType() != MenuType::MENU) return;
        clearSelectedSlice();
	}       
    ...
    void draw(uiVizWidgetContext context) override {
        ...
        *-*-*-*-*-**-*-*-*-*-**-*-*-*-*-**-*-*-*-*-**-*-*-*-*-**-*-*-*-*-**-*-*-*-*-*/
        for (int i=0; i<mMenuCollectionSliceData.size(); i++) {  
            ...
            slice.collectionItem.icon.setColor((sliceHovered || slice.collectionItem.isSelected ? 
                slice.selectedFontColor : 
                slice.regularFontColor
            ));            
            ...
            if (sliceHovered) {
                ...
                if (!mHasJustChangedItemVisibility) {
                    ...
                    if ((anySliceSelected && !slice.collectionItem.isSelected) || pressedAndReleased) {
                        ...
                        if(slice.collectionItem.isSelected && getMenuType() == MenuType::MENU) { // Don't want to toggle with tabs...
                            uiVizWidgetMenuCollectionArgs args(slice.collectionItem.tag, nullptr, NULL,NULL);
            ...
            ofPushStyle();                  
            ofSetColor((sliceHovered || slice.collectionItem.isSelected ? 
                slice.selectedFontColor : 
                slice.regularFontColor
            ));            
	...
	virtual void onCoreWidgetMouseReleased(ofMouseEventArgs &e) override {
        ...
        if (parentWidget()) return; // Only want to apply below logic if a root widget!

        if (getSelectedTag() != "") {
            for (uiVizWidgetMenuCollectionItemSlice& slice:mMenuCollectionSliceData) {
                uiVizWidgetMenu* m = dynamic_cast<uiVizWidgetMenu*>(slice.collectionItem.menu);
                if (slice.collectionItem.menu && (
                        slice.collectionItem.menu->hasActiveChildWidgetOrElement(true) || 
    ...
    virtual void onWidgetAddedAsChild(uiVizWidgetEventArgs args) override {    
        if (parentWidget()) {
            ...
            uiVizWidget* owner = getOwnerWidget();
            if (owner) {
                if (getItemXExpression() == "") setItemXExpression("${" + owner->getPersistentId() + ".ABSOLUTE_USABLE_X}");
                if (getItemYExpression() == "") setItemYExpression("${" + getPersistentId() + ".BOTTOM}");
                if (getItemWidthExpression() == "") setItemWidthExpression("${" + owner->getPersistentId() + ".USABLE_WIDTH}");
                if (getItemHeightExpression() == "") setItemHeightExpression("${" + getPersistentId() + ".BOTTOM_USABLE_HEIGHT}");
        ...
        if (collectionItem.menu) {
            ...
            if (getMenuType() == MenuType::TAB) {
                ...
                if (w) {
                    ...
                    if (collectionItem.menu->getX_Expr() == "") collectionItem.menu->setX_Expr("${" + w->getPersistentId() + ".ABSOLUTE_USABLE_X}");
                    if (collectionItem.menu->getY_Expr() == "") collectionItem.menu->setY_Expr("${" + getPersistentId() + ".BOTTOM}");
                    if (collectionItem.menu->getWidth_Expr() == "") collectionItem.menu->setWidth_Expr("${" + w->getPersistentId() + ".USABLE_WIDTH}");
                    if (collectionItem.menu->getHeight_Expr() == "") collectionItem.menu->setHeight_Expr("${" + getPersistentId() + ".BOTTOM_USABLE_HEIGHT}");
