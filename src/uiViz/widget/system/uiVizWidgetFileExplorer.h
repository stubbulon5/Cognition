...
public:
    ...
    virtual void setPath(string val) {
        ...
        if (mPath.size() > 0 && mPath[0] == '[') {
            ...
            if(!ofDirectory::doesDirectoryExist(mPath, false)) mPath = ofFilePath::getUserHomeDir();
        ...
        if (fileLocationsList) fileLocationsList->setPath(mPath);
        if (fileList) fileList->setPath(mPath);
    ...
    virtual void onWidgetEventReceived(uiVizWidgetEventArgs &args) override {
        ...
        if (args.eventName == WIDGET_EVENT::CLICK && breadcrumb && args.sender.getPersistentId() == breadcrumb->getPersistentId()) {
            ...
            #ifdef TARGET_WIN32
            if (key.find("\\", 1)) key = key.replace(0, 1, ""); // remove the leading "\"" for windows path 
            #endif
