...
public:
    ...
    virtual ~uiVizWidgetDebug() {
        if (debugTabs) ofRemoveListener(debugTabs->menuItemSelected, this, &uiVizWidgetDebug::onMenuItemSelected);
    }
    
    bool needsUpdateBeforeDraw(bool shouldResetTimer) override {
        if (getTimerMillis() >= 500) { // refresh debug info every 1/2 second ...
    ...
    void update(uiVizWidgetContext context) override {
        if (!mWidgetMapRef  || mWidgetMapRef->size() == 0) return;
        debugInfo = debugTemplate;
        setTitle("DEBUG INFO: [" + ofToString(ofGetFrameRate(), 0) + "fps]");
        
        int LEFT_PADDING = 50;

        ofGetFrameRate() < getViz()->getUserExperience() ?
            getTheme().setTitleFontColor(ofColor(255, 0, 0, 200)) : getTheme().setTitleFontColor(ofColor(0, 0, 0, 200));
        ...
        try {
            ...
            if (prior) ofStringReplace(debugInfo, "${PRIOR}", prior ? prior->getPersistentId() + " " + prior->getWidgetId() : "---");
            if (activeChild) ofStringReplace(debugInfo, "${ACTIVE_CHILD}", activeChild ? activeChild->getPersistentId() + " " + activeChild->getWidgetId() : "---");
            if (priorChild) ofStringReplace(debugInfo, "${PRIOR_CHILD}", priorChild ? priorChild->getPersistentId() + " " + priorChild->getWidgetId() : "---");
            if (target) ofStringReplace(debugInfo, "${TARGET}", target ? target->getPersistentId() + " " + target->getWidgetId() : "---");
            if (nextInlineForFocus) ofStringReplace(debugInfo, "${NEXT_FOR_FOCUS}", nextInlineForFocus->getPersistentId());
        ...
        for (int i=mWidgetMapRef->size() -1; i>=0; i--) {
            ...
            if ( widget.getPersistentId() != "APP_DEBUG_INFO") { // fix this ... (debug widget is showing some weird numbers)
    ...
    void draw(uiVizWidgetContext context) override {
        ...
        if (debugTabs->getSelectedTag() != "DEBUG") return;
        ofPushStyle();   
    ...
    void addToOscReceiveLog(ofxOscMessage& m) {
        ...
        if (eventSenderId == getPersistentId() || eventSenderId == "OSCSENDLOG" || eventSenderId == "OSCRECEIVELOG") return; // dont log osc events for debug widget!

        oscReceiveLog->addRow(
            uiVizWidgetTableRow("HEADER", {
        ...
        if (!loaded) {
         ...
         //   addChildWidget(*oscSendLog, true);            
           ...
           debugTabs = new uiVizWidgetMenuCollection("DEBUG_TABS", R"(
            ...
            debugTabs->setMenuCollection( {
                uiVizWidgetMenuCollectionItem("DEBUG", nullptr, "DEBUG", uiVizIconCache::getIcon("REG_BUG")),
