...
public:
    ...
    void setValue(float val, bool fireEvent) {
        ...
        if (fireEvent && valChanged) {
            ...
            if (parent()) parent()->eventTransmit(getPersistentId(), WIDGET_EVENT::VALUE_CHANGED, "<event><value>" + eventVal + "</value><percent>" + eventPerc + "</percent></event>", *this);
        }
    ...
    virtual void update(uiVizWidgetContext context) override {
        ...
        if (getIsProgressing()) {
            // Slider BG
            getIsRoundedRectangle() ? vizElm_BG.setRectRounded(getSliderBG_x(), scale(getY()), scale(mSliderBGSize), scale(getHeight()), scale(5), getTheme().ElementBackgroundColor_withAlpha(getCurrentBackgroundAlpha())) :
            vizElm_BG.setRectangle(mSliderBG_x, scale(getY()), scale(mSliderBGSize), scale(getHeight()), getTheme().ElementBackgroundColor_withAlpha(getCurrentBackgroundAlpha()));
            
            // Slider
            if (getIsAsyncProgressBar()) {
                if (getIsDescending()) {
                    getIsRoundedRectangle() ? vizElm_SLIDER.setRectRounded(sliderX+scale(mSliderBGSize)-mSliderCurrentWidth, sliderY, mSliderCurrentWidth, sliderHeight, scale(5), getTheme().ElementForegroundColor_withAlpha(getCurrentForegroundAlpha())) :
                    vizElm_SLIDER.setRectangle(sliderX+scale(mSliderBGSize)-mSliderCurrentWidth, sliderY, mSliderCurrentWidth, sliderHeight, getTheme().ElementForegroundColor_withAlpha(getCurrentForegroundAlpha()));
                } else {
                    getIsRoundedRectangle() ? vizElm_SLIDER.setRectRounded(sliderX, sliderY, mSliderCurrentWidth, sliderHeight, scale(5), getTheme().ElementForegroundColor_withAlpha(getCurrentForegroundAlpha())) :
                    vizElm_SLIDER.setRectangle(sliderX, sliderY, mSliderCurrentWidth, sliderHeight, getTheme().ElementForegroundColor_withAlpha(getCurrentForegroundAlpha()));
                }
