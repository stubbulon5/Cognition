...
public:
    ...
    virtual void update(uiVizWidgetContext context) override {
        ...
        getIsRoundedRectangle() ? vizElm_TEXTAREA.setRectRounded(getViz()->scale(getX()), getViz()->scale(getY()), getViz()->scale(getWidth()), getViz()->scale(getHeight()), scale(5), getTheme().ElementBackgroundColor_withAlpha(getCurrentBackgroundAlpha())) :
        vizElm_TEXTAREA.setRectangle(getViz()->scale(getX()), getViz()->scale(getY()), getViz()->scale(getWidth()), getViz()->scale(getHeight()), getTheme().ElementBackgroundColor_withAlpha(getCurrentBackgroundAlpha()));
        ...
        if (getIsFormatted()) {
            ...
            r = getViz()->getFonts(getTheme())->drawFormattedColumn(
                getValue(),
                scale(getUsableWidth()*-1-100),
                scale(getUsableY() + getNonScaledPadding()),
                scale(getUsableWidth() - getViz()->getNonScaledPadding()*2.0f)
            );
            ofPopStyle(); 
        } else {
            defaultStyle.color = fontColor;
            r = getViz()->getFonts(getTheme())->drawColumn(
                getValue(),
                defaultStyle,
                scale(getUsableWidth()*-1-100),
                scale(getUsableY() + getNonScaledPadding()),
