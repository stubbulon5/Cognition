  ...
  // doxygen
    ...
    */
    ...
    virtual void update(uiVizWidgetContext context) override {
        ...
        // Slider
        getIsRoundedRectangle() ? vizElm_SLIDER.setRectRounded(sliderX, sliderY, scale(mSliderSize), sliderHeight, scale(5), getTheme().ElementBackgroundColor_withAlpha(getCurrentBackgroundAlpha())) :
        vizElm_SLIDER.setRectangle(sliderX, sliderY, scale(mSliderSize), sliderHeight, getTheme().ElementBackgroundColor_withAlpha(getCurrentBackgroundAlpha()));
           ...
           //  getViz()->scale(getUsableY() + getHeight()/2.0f),
            ...
            if (getShowValueWhenDragging() && getIsElmDragging()) {
                ...
                ofRectangle r = getViz()->getFonts(getTheme())->drawColumn(
                     ...
                     scale(getUsableY() + getHeight()/2.0f),
                     (getPerc() > 0.5f) ? vizElm_BG.getRectangle().width : vizElm_BG.getRectangle().width - getScaledPadding(),
                     (getPerc() > 0.5f) ? ofAlignHorz::OF_ALIGN_HORZ_LEFT : ofAlignHorz::OF_ALIGN_HORZ_RIGHT,
                     false);
            }
        }
