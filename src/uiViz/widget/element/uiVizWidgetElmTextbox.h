...
public:
    ...
    string getTextTypeString(TextType val) {
        switch(val) {
            case TextType::MULTI_LINE: return "MULTI_LINE";
            case TextType::NUMERIC: return "NUMERIC";
            case TextType::PASSWORD: return "PASSWORD";
            case TextType::FORMATTED: return "FORMATTED";
            default: return "SINGLE_LINE";
        }
    };
    
    TextType getTextTypeEnum(string val) {
        val = ofToUpper(val);
        if (val == "MULTI_LINE") return TextType::MULTI_LINE;
        if (val == "NUMERIC") return TextType::NUMERIC;
        if (val == "PASSWORD") return TextType::PASSWORD;
        if (val == "FORMATTED") return TextType::FORMATTED;
    ...
    void setValue(string val, bool fireEvent) {
        ...
        if (getTextType() == TextType::NUMERIC) {
            if ((val != "" && val != "0" && val.substr(0, 1) != "0")) {
                if (std::atof(val.c_str()) == 0) return;
            }
        }
        ...
        if (fireEvent && valChanged) {
            ...
            if (parent()) parent()->eventTransmit(getPersistentId(), WIDGET_EVENT::VALUE_CHANGED, "<event><value>" + eventVal + "</value></event>", *this);
        }
    ...
    virtual void update(uiVizWidgetContext context) override {
        ...
        getIsRoundedRectangle() ? vizElm_TEXTBOX.setRectRounded(scale(getX()), scale(getY()), scale(getWidth()), scale(getHeight()), scale(5), getTheme().ElementBackgroundColor_withAlpha(getCurrentBackgroundAlpha())) :
        vizElm_TEXTBOX.setRectangle(scale(getX()), scale(getY()), scale(getWidth()), scale(getHeight()), getTheme().ElementBackgroundColor_withAlpha(getCurrentBackgroundAlpha()));
 ...
 */
        ...
        ofRectangle r = getViz()->getFonts(getTheme())->drawColumn(
            getTextOrTitle(),
            defaultStyle,
            scale(getUsableX()),
            scale(getUsableY()),
            scale(getUsableWidth() - getNonScaledPadding()*2.0f),
            ofAlignHorz::OF_ALIGN_HORZ_LEFT,
            false);
        ofPopStyle();                                                          
    ...
    virtual void onWidgetKeyPressed(ofKeyEventArgs &e) override {
        ...
        if (!getIsFocused(true)) return;
        
        int key = e.key;
        
        if (!keyIsValid(key, e)) return;
        
        if (key != OF_KEY_BACKSPACE && key != OF_KEY_LEFT && key != OF_KEY_RIGHT &&
            key != OF_KEY_UP && key != OF_KEY_DOWN && !moreCharactersAllowed()) return;
        if (mHighlightText) {
        ...
        if (key == OF_KEY_BACKSPACE){
            ...
            if (key == OF_KEY_UP || key == OF_KEY_DOWN) return;
    ...
    void pasteAtIndex(int index, string paste, bool updateCursor) {
        ...
        if (updateCursor) setCursorIndex(index + paste.size());
        calculateTextBounds();
    ...
    int getIndexForLine(int currIndex, string mode) {
        ...
        if (mode == "DOWN" && mCursorLineIndex < textLines.size() + 1 ) {
            if (textLines.size() == 1) return textLines[0].size() - 1; // jump to end of line (only 1 line of text!)
    ...
    void setCursorIndex(int index) {
        ...
        mCursorY =  scale(getUsableY()) + bbox.height - 
            (laidOutLines.size() > 0 ? scale(mCursorHeight)*2.0f : getTextOrTitle().size() > 0 ? scale(mCursorHeight) : 0) + 
            scale(mCursorHeight)*0.22f;
        mCursorLineIndex = laidOutLines.size();
        ensureCursorIsInView();
        

        if (mCursorX_ExceededBounds) mCursorX = mOldCursorX;
        if (mCursorY_ExceededBounds) mCursorY = mOldCursorY;
        if (mCursorX_ExceededBounds || mCursorY_ExceededBounds) return;
