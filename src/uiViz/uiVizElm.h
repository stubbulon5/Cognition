...
public:
    ...
    struct vizRect {
        ...
        vizRect(int x, int y, int width, int height) :
        x(x), y(y), width(width), height(height) {};
        
        vizRect(int x, int y, int width, int height, float topLeftRadius, float topRightRadius, float bottomRightRadius, float bottomLeftRadius) :
        x(x), y(y), width(width), height(height),
    ...
    void setRectGradient(ofColor topRight, ofColor bottomRight, ofColor bottomLeft, ofColor topLeft) {
        ...
        for (int i = 0; i < numVert; i++) {
            ...
            if (v.x < x_half && v.y < y_half) { // top left
                mMesh.addColor(topLeft);
            } else if (v.x > x_half && v.y < y_half) { // top right
                mMesh.addColor(topRight);
            } else if (v.x > x_half && v.y > y_half) { // bottom right	
                mMesh.addColor(bottomRight);
            } else if (v.x < x_half && v.y > y_half) { // bottom left	
    ...
    void draw() {
        if (mPath.isFilled()) {
            ...
            ofPushStyle();
            if (mWasColorSet && !mWasGradientSet) ofSetColor(mColor);
    ...
    struct vizCircle {
        ...
        vizCircle(int x, int y, int radiusX, int radiusY, bool filled) :
        x(x), y(y), radiusX(radiusX), radiusY(radiusY), filled(filled) {};
    ...
    struct vizTriangle {
        ...
        vizTriangle(float x1,float y1,float x2,float y2,float x3, float y3) :
