        ...
        if (themesElm != NULL) {
            for(TiXmlNode* themeElm = themesElm->FirstChild(); themeElm; themeElm = themeElm->NextSibling() ) {
                ...
                if (currTheme.load(themeXML)) {
                    ...
                    if (currTheme.Name == "System") mSystemTheme = currTheme;
                    if (currTheme.Name == "SystemDark") mSystemThemeDark = currTheme;
                }
            }
            return true;
        }
...
public:
    ...
    struct AnimationData {
        ...
        AnimationData(string widgetId, string animationName, float startVal, float targetVal, float secondsDuration) :
            widgetId(widgetId), animationName(animationName), startVal(startVal), targetVal(targetVal), secondsDuration(secondsDuration) {};
    };
    ...
    void removeWidgetId(string widgetId) {
        ...
        if (itr != mWidgetIds.end()) mWidgetIds.erase(itr);
    }
    ...
    void setIsPopoutVisible(string popoutWidgetId, bool val) {
        ...
        if (itr != mVisiblePopouts.end()) mVisiblePopouts.erase(itr);
        if (val) mVisiblePopouts.push_back(popoutWidgetId);
    } 
    ...
    void setModalWidgetId(string widgetId) {
        if (widgetId != mModalWidgetId) mPriorModalWidgetId = mModalWidgetId;
    ...
    struct WidgetEvent {
        ...
        WidgetEvent(string eventSenderId, string eventTargetId, string eventName, string redirectEventName="") :
           eventSenderId(eventSenderId), eventTargetId(eventTargetId), eventName(eventName), redirectEventName(redirectEventName) {};

        WidgetEvent(string address, string eventSenderType, string eventSenderId, string eventTargetId, string eventName, string redirectEventName, string eventXML) :
            address(address), eventSenderType(eventSenderType), eventSenderId(eventSenderId), eventTargetId(eventTargetId), eventName(eventName), redirectEventName(redirectEventName), eventXML(eventXML) {};
    };  
    ...
    void addToWidgetEventListenMap(string eventListenerId, string eventSourceId, string eventName, string redirectEventName="", bool allowDuplicates = false) {
         bool found = false;
         if(eventListenerId == eventSourceId || eventListenerId == "" || eventSourceId == "" || eventName == "") return;
         if (!allowDuplicates) {
            for(WidgetEvent e:mEventListenMap[eventSourceId]) {
    ...
    vector<WidgetEvent> getListeningEvents(string eventListenerId) { // The list of events this widget is listening to
    ...
    void removeListeningEvents(string eventListenerId) { // The list of events this widget is listening to
    ...
    vector<WidgetEvent> getListenEvents(string eventSourceId) { // List of events listening to this widget
...
private:
    ...
    AnimationData getAnimation(string name) {
        for(auto animation:mAnimations){
            if (animation.animationName==name) return animation;
        }
	...
	float scaleForUserExperience(string widgetId, string animationName, float currVal, float targetVal, float secondsDuration, bool includeResolutionMultiplier) {
        ...
        #if VIZ_DEBUG_LEVEL >= 0 && VIZ_DEBUG_LEVEL < 1 
			isDecreasing ? cout << "[ANIMATION: " << animation.animationName << "] decreasing " << currVal << " + " << newVal << endl :
				cout << "[ANIMATION: " << animation.animationName << "] INC " << currVal << " + " << newVal << endl;
        #endif     
		return retVal;
