...
uiVizWidget* uiVizWidgetManager::loadWidget(string WIDGET_CLASS, string persistentId, string widgetXML) {
    ...
    if (!wasFoundInMap) widget = new uiVizWidget(persistentId, widgetXML);
...
void uiVizWidgetManager::recalculateWidgetExpressions(uiVizWidget &w) {
    ...
    if (w.getIgnoreExpr_Layout()) return;
    ...
    if (w.getX_Expr() != "" || w.getY_Expr() != "" || w.getWidth_Expr() != "" || w.getHeight_Expr() != "") {
        ...
        // Self
        if (exprStringX != "") w.replaceExpressions(&w, exprStringX);
        if (exprStringY != "") w.replaceExpressions(&w, exprStringY);
        if (exprStringWidth != "") w.replaceExpressions(&w, exprStringWidth);
        if (exprStringHeight != "") w.replaceExpressions(&w, exprStringHeight);
        
        for(uiVizWidget &exprWidget:mWidgets) {
            if (exprWidget.getIsVisible() && w.getPersistentId() != "") {
                if (exprStringX != "") w.replaceExpressions(&exprWidget, exprStringX);
                if (exprStringY != "") w.replaceExpressions(&exprWidget, exprStringY);
                if (exprStringWidth != "") w.replaceExpressions(&exprWidget, exprStringWidth);
                if (exprStringHeight != "") w.replaceExpressions(&exprWidget, exprStringHeight);
            }
        }
    ...
    *-*-*-*-*-**-*-*-*-*-**-*-*-*-*-**-*-*-*-*-**-*-*-*-*-**-*-*-*-*-**-*-*-*-*-*/
    ...
    while (uiVizShared::getViz()->hasOscEventsInTransmitQueue()) {  
        ...
        if (m.getAddress() != "") {
            ...
            if (SHOW_DEBUG_INFO) {
                dynamic_cast<uiVizWidgetDebug*>
                    (uiVizWidgetManager::getWidgetByPersistentId("APP_DEBUG_INFO"))->addToOscSendLog(m);
            }    
    ...
    while(uiVizShared::getViz()->hasOscEventsInReceiveQueue()) {
        ...
        if (SHOW_DEBUG_INFO) {
             dynamic_cast<uiVizWidgetDebug*>
                (uiVizWidgetManager::getWidgetByPersistentId("APP_DEBUG_INFO"))->addToOscReceiveLog(m);
        }        
...
bool uiVizWidgetManager::checkShouldCloseOrDelete(uiVizWidget* widget) {
    ...
    if (widget->shouldCloseThisWidget()) {
        ...
        if (widget->getOwnerWidgetId() != "") {
            ...
            if (widgetOwner) widgetOwner->onWidgetOwnedWidgetClosed(widget);
        }
    }    
        ...
        for(uiVizWidget &widget:mWidgets) {
            if (exceptWidgets.size() > 0 && exceptWidgets[0] != nullptr) {
                if(!widget.matchesWidget(exceptWidgets)) {
                    if (widget.getWidgetClassType() == WIDGET_CLASS_DEBUG) continue;
                    if (widget.getPersistentId() == "CLIPBOARD_MENU" || widget.getPersistentId() == "CLIPBOARD") continue;
...
bool uiVizWidgetManager::removeWidget(uiVizWidget &w) {
    for(uiVizWidget &widget:mWidgets){
        if (widget.getWidgetId() == w.getWidgetId()) {
            if(w.getIsPermanentWidget()) return false;
            widget.clearIfModal();
...
void uiVizWidgetManager::highlightWidget(string persistentId, bool highlight, bool clearOtherHighlights) {
    ...
    if(w != nullptr) w->setIsHighlighted(highlight);
}
...
uiVizWidget* uiVizWidgetManager::getFrontWidget() {
    if(mWidgets.size() == 0) return nullptr;
...
uiVizWidget* uiVizWidgetManager::getWidget(uiVizWidget* parentWidget, string widgetId) {
    ...
    */
    ...
    if (parentWidget->getWidgetId() == widgetId) return parentWidget;
...
uiVizWidget* uiVizWidgetManager::getWidgetByPersistentId(uiVizWidget* parentWidget, string persistentId) {
    if (ofToUpper(parentWidget->getPersistentId()) == persistentId) return parentWidget;
    for(uiVizWidget &childWidget:parentWidget->getChildWidgets()){
...
uiVizWidgetBase* uiVizWidgetManager::getWidgetBase(string widgetId, uiVizWidget &searchInsideWidget) {
    ...
    if (childWidgetCount(searchInsideWidget) > 0) {
        for(uiVizWidget &childWidget:searchInsideWidget.getChildWidgets()){
            ...
            if (searchChildWidget != nullptr) return searchChildWidget;
...
uiVizWidgetBase* uiVizWidgetManager::getWidgetBase(string widgetId) {
    for(uiVizWidget &widget:mWidgets){
        ...
        if (searchWidget != nullptr) return searchWidget;        
    }
    return nullptr;
}
...
uiVizWidget* uiVizWidgetManager::getActiveParentWidget() {
    for (int i=mWidgets.size() -1; i>=0; i--) {
        uiVizWidget &widget = mWidgets[i];
        if (widget.getIsModalWidget()) return &widget;
        if (widget.getIsActiveWidget() || hasActiveChildWidgets(widget)) return &widget;
    }
    return nullptr;
}
...
bool uiVizWidgetManager::save(string fileName) {
    string widgetCollection = "<widgets version=\"" + applicationProperties.applicationVersion
        + "\" title=\"" + currentProjectProperties.projectName + "\">";
...
void uiVizWidgetManager::showClipboardPopupMenu(int x, int y, string clipboardOperationTargetWidgetId) {
    ...
    if (clipboardMenu->getMenu()->getIsVisible()) return;
...
void uiVizWidgetManager::hideModal() {
    ...
    if(delWidget) removeWidget(*delWidget);
}
